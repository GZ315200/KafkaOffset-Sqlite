...
import com.unistack.tamboo.commons.json.GenericRecord;
import com.unistack.tamboo.commons.json.JsonConverter;
...
//        this.tasks = new Table<>();
    ...
    @Override
    public void configure(RuntimeConfig config) {
        this.topic = config.getString(RuntimeConfig.STATUS_STORAGE_TOPIC);
    ...
    private KafkaTopicBaseLog<String, byte[]> createKafkaBasedLog(String topic, Map<String, Object> producerProps,
        ...
        return new KafkaTopicBaseLog<>(producerProps, consumerProps, consumedCallback, topic, time, createTopics);
    }


    private void read(ConsumerRecord<String, byte[]> record) {
        String key = record.key();
        if (key.startsWith(RUNNER_STATUS_PREFIX)) readRunnerStatue(key, record.value());
        else log.warn("Discarding record with invalid key {}", key);
    }
...
//        RunnerStatus status = new RunnerStatus("1", AbstractStatus.State.DESTROYED, "", 4);
//        System.out.println(status);

//        System.out.println(parseConnectorStatusKey("status-runner-1"));

        Map<Object, Object> data = Maps.newHashMap();

        JsonConverter jsonConverter = new JsonConverter();

        data.put(STATE_KEY_NAME, "b");
        data.put(TRACE_KEY_NAME, "ok");
        data.put(GENERATION_KEY_NAME, 1);
        data.put(RUNNER_ID_KEY_NAME, "a");

        byte[] value = jsonConverter.fromConnectData("test", data.toString().getBytes());
        System.out.println(jsonConverter.fromConnectData("test", data.toString().getBytes()));

        System.out.println(jsonConverter.toConnectData("test", value));
