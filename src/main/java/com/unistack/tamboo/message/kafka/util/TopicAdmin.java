...
public class TopicAdmin implements AutoCloseable {
    ...
    public Set<String> createTopics(NewTopic... topics) {
        ...
        if (topics != null) {
            for (NewTopic topic : topics) {
                if (topic != null) topicByName.put(topic.name(), topic);
            }
        }
    ...
    public List<Integer> listBrokerIds() {
        ...
        ClusterDescription description = listCluster();
        description.getClusterNode().forEach(clusterNode -> {
            brokerIds.add(Integer.valueOf(clusterNode.getIdString()));
        });
        return brokerIds;
    }
     ...
     * @return
     */
    public Map<Integer, Map<String, DescribeLogDirsResponse.LogDirInfo>> describeLogDir(List<Integer> brokerIds) {
    ...
    public ConfigResource alertBrokersSaslConfig(String username, String privateKey) {
        ConfigResource key = null;
        String bootstrapServers = bootstrapServers();
        Map<ConfigResource, Config> configs = Maps.newHashMap();
        List<ConfigEntry> entries = Lists.newArrayList();
        ConfigEntry entry = new ConfigEntry("sasl.jaas.config", jaasConfigProperty("PLAIN", username, privateKey).value());
        entries.add(entry);
    ...
    public ConfigResource alertClientSaslConfig(String topic, String username, String privateKey) {
        ConfigResource key = null;
        String bootstrapServers = bootstrapServers();
        Map<ConfigResource, Config> configs = Maps.newHashMap();
        List<ConfigEntry> entries = Lists.newArrayList();
